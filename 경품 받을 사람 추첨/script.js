...
historyBtn.addEventListener('click', () => {
    ...
    if (winners.length === 0) {
        historyListEl.innerHTML = '<div style="text-align: center; color: rgba(255,255,255,0.5); padding: 40px 20px;">아직 당첨 내역이 없습니다.</div>';
    } else {
        winners.forEach((winner, index) => {
            ...
            li.innerHTML = `
                <div class="history-item-info">
                    <span class="history-item-prize">${index + 1}회차 - ${winner.prize}</span>
                    <span class="history-item-name">🎉 ${winner.name}</span>
                </div>
            `;
    ...
    historyDisplayEl.style.display = 'flex';
});

historyCloseBtn.addEventListener('click', () => {
    historyDisplayEl.style.display = 'none';
});
